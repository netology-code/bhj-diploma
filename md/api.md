...
**Важный момент!** При работе с локальным сервером логика работы должна быть немного другая: Вместо объекта `data` используйте `id` сущности в запросе. Пример получения определённого счёта: `http://localhost:8000/account/2`
